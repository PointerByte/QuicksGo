...
func (symmetricRepository) EncryptAES(symmetricalAccess, value, additionalData string) (string, error) {
	...
	cipherText := gcm.Seal(nil, nonce, []byte(value), []byte(additionalData))
	payload := append(nonce, cipherText...)
	return base64.StdEncoding.EncodeToString(payload), nil
}

// DecryptAES decrypts ciphertext generated by EncryptAES.
func (symmetricRepository) DecryptAES(symmetricalAccess, cipherValue, additionalData string) (string, error) {
	aesKeyBytes, err := base64.StdEncoding.DecodeString(symmetricalAccess)
	if err != nil {
		return "", fmt.Errorf("decode AES key: %w", err)
	}
...
// provided Base64 key.
func (symmetricRepository) EncodeFernet(keyString, value string) (string, error) {
...
func (symmetricRepository) DecodeFernet(keyString, cipherValue string) (string, error) {
	key, err := decodeFernetKey(keyString)
	if err != nil {
		return "", err
	}

	token, err := base64.URLEncoding.DecodeString(cipherValue)
	if err != nil {
		return "", err
	}
...
// encoded private key.
func (asymmetricRepository) RSA_OAEP_Decode(key, cipherText string) (string, error) {
