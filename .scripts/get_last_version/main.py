...
def build_suggested_tags(
    ...
    for module in modules:
        if not module_has_changes(module, changed_paths):
            continue
        remote_tag = latest_tag_for_module(remote_tags, module)
        local_tag = latest_tag_for_module(local_tags, module)
        suggestions[module.module_path] = bump_version(remote_tag or local_tag, bump_type, module.tag_prefix)
...
def update_go_mod_dependencies(
    ...
    for module in modules:
        go_mod_path = PROJECT_ROOT / module.directory / "go.mod" if module.directory != "." else PROJECT_ROOT / "go.mod"
...
def format_report(
    ...
    for module in modules:
        ...
        remote_tag = latest_tag_for_module(remote_tags, module)
        local_tag = latest_tag_for_module(local_tags, module)
        suggested_tag = suggested_tags.get(module.module_path, "-") if has_changes else "-"
    ...
    if suggested_count == 0:
        lines.append("- No hay modulos con cambios locales.")
        lines.append("")
        lines.append("Comandos git sugeridos:")
        lines.append("- No hay comandos para generar tags porque no hay cambios locales.")
        return "\n".join(lines)
...
def main() -> int:
    ...
    if args.update_go_mod:
        updated_go_mod_files = update_go_mod_dependencies(MANAGED_MODULES, suggested_tags)
